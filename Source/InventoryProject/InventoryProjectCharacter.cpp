...
void AInventoryProjectCharacter::BeginPlay()
{
	...
	{
		// UE_LOG(LogTemp, Warning, TEXT("AInventoryProjectCharacter::BeginPlay: Controller is %s"), *CharacterController->GetActorNameOrLabel());
	}

	// UE_LOG(LogTemp, Warning, TEXT("[DBG PAWN BeginPlay] Pawn=%s Owner=%s Instigator=%s Controller=%s"),
	// *GetName(),
	// * (GetOwner() ? GetOwner()->GetName() : FString("None")),
	// * (GetInstigator() ? GetInstigator()->GetName() : FString("None")),
	// * (GetController() ? GetController()->GetName() : FString("NoController")));
}
